...
- Deploy: **2 Docker images** — `popyog/sheetsubmit-testmycode-api:latest` (Express API) and
  `popyog/sheetsubmit-testmycode-web:latest` (nginx serving `frontend/dist`, proxying
...
8. **Deploy flow:** after a change is done and the user wants it live, run `redeploy.bat`
   from this repo root — it builds + pushes both images (Railway auto-deploys the pushed
   images). Do not skip it; ask first only if the user hasn't asked to ship.
