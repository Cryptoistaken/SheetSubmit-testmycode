...
- Autosave: 300 ms debounce. **Everything is sent incrementally**: `PUT /files/:id/append` carries `{ base, ops, newLogs, undoNew, redoNew, dataCount }` — only changed cells + only log/undo/redo entries not yet on the server (client tracks `logBase`/`undoBase`/`redoBase`). Server RPUSHes onto `logs:`/`undo:`/`redo:` LISTS (capped 500/100/100, migrated from legacy JSON blobs on first read). Every file has a `seq` counter (returned by `/full`, `/persist`, `/append`). Structural/action saves (merge/replace/append/clean/bubble, undo/redo, paste, removeEmptyRows, admin) use full `/persist` (replace semantics). On append `409 version conflict` the client refetches `/full`, re-applies its unsent ops onto the server rows (local edits survive), bumps `lastSeq`, and re-appends.
