...
export default function BubbleDesignPage(){
  ...
  const drag=useRef({dragging:false,moved:false,sx:0,sy:0,ox:0,oy:0});
  const [claudeTick,setClaudeTick]=useState(0);
  const fireClaude=()=>{ if(state.icon==="claudeCode") setClaudeTick(t=>t+1); };
  ...
  useEffect(()=>{
    ...
    const onDown=(e:PointerEvent)=>{
      ...
      drag.current={dragging:true,moved:false,sx:e.clientX,sy:e.clientY,ox:r.left,oy:r.top};
    ...
    const onMove=(e:PointerEvent)=>{
      ...
      if(drag.current.moved){
        const nx=Math.max(4,Math.min(window.innerWidth - el.offsetWidth -4, drag.current.ox+dx));
        const ny=Math.max(4,Math.min(window.innerHeight - el.offsetHeight -4, drag.current.oy+dy));
  ...
  const bubbleInner = state.icon==="claudeCode" ? CLAUDE_CODE_SVG : (ICONS[state.icon] ?? ICON_CLAUDE);
  const bubbleTip = state.icon==="claudeCode" ? "drag me — animated" : "drag me";
  ...
  return (
    <>
      ...
      <div ref={bubbleRef} className="bd-bubble" aria-label="Floating bubble preview (demo only)" title={state.icon==="claudeCode" ? "Tap to animate — demo only" : undefined} style={{width:state.size,height:state.size,color:state.color,...bubbleStyle}} onClick={fireClaude} onPointerDown={fireClaude}>
        {state.icon==="claudeCode" ? (
          <div key={claudeTick} style={{width:"100%",height:"100%",color:state.color}} dangerouslySetInnerHTML={{__html:CLAUDE_CODE_SVG}} />
        ) : (
          <div style={{width:"100%",height:"100%",color:state.color}} dangerouslySetInnerHTML={{__html:bubbleInner}} />
        )}
